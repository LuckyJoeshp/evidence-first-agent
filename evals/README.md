...
The harness compares response quality, not just length. Its central regression
target is evidence loss: a response must remain readable without dropping or
relabeling decision-critical facts. Cases live in `cases.jsonl`; the scoring
contract lives in `rubric.md`.
...
python3 scripts/run_evals.py run \
  --runner claude \
  --condition candidate \
  --condition-skill skills/evidence-first/SKILL.md \
  --trials 3 \
  --budget-usd 12.50 \
  --output evals/results/responses.jsonl
```
...
Both example runners isolate the call from the operator's own agent
configuration: `--setting-sources ""` for Claude, `--ignore-user-config
--ephemeral` for Codex. Keep that isolation when adding runners: without it,
user-level plugins, hooks, memory, and output styles leak into every condition.
The sharpest case is this repo's optional always-on flag
(`~/.claude/.evidence-first-always`), which would inject the candidate rules
into the **baseline** condition and make the comparison measure the skill
against itself.
...
```json
{"case_id":"direct-answer","trial":1,"condition":"candidate","correctness":5,"evidence_integrity":5,"autonomy":5,"actionability":5,"safety":5,"clarity":5,"blocker":false,"notes":"Direct and correct."}
```
...
Do not claim effectiveness from harness unit tests alone; publish paired
responses and blinded scores.
